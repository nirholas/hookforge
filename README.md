...
| [`BlockBatchClearing`](contracts/src/hooks/BlockBatchClearingHook.sol) | Clears a whole block's orders at one uniform price, solved in closed form on-chain with no solver and no auction, so being first is worth nothing and opposing orders fill against each other with no slippage. | shipped |
